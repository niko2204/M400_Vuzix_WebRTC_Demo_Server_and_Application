...
import static edu.p2p.OwtP2PError.P2P_CLIENT_ILLEGAL_ARGUMENT;
import static edu.p2p.OwtP2PError.P2P_CONN_SERVER_UNKNOWN;
...
public class SocketSignalingChannel implements SignalingChannelInterface {
    private static final String TAG = "VuzixSocketCl";
    private final String CLIENT_CHAT_TYPE = "owt-message";
    private final String VUZIX_MESSAGE_TYPE = "vuzix-message";
    ...
    private VuzixChannelObserver vuzixChannelObserver;
    ...
    public void setVuzixChannelObserver(VuzixChannelObserver v){
        this.vuzixChannelObserver = v;
    }

    private Listener onVuzixMessageCallback = args -> {
        Log.d(TAG,"onVuzixMessageCallback");
        JSONObject argumentJsonObject = (JSONObject) args[0];
        if(vuzixChannelObserver != null){
            try {
                vuzixChannelObserver.onVuzixMessage(argumentJsonObject.getString("from"),
                        argumentJsonObject.getString("data"));
            } catch (JSONException e) {
                e.printStackTrace();
            }
    ...
    public void connect(String userInfo, ActionCallback<String> callback) {
        ...
        try {
            ...
            socketIOClient.on(Socket.EVENT_CONNECT_ERROR, onConnectErrorCallback)
                    ...
                    .on(VUZIX_MESSAGE_TYPE, onVuzixMessageCallback)
    ...
    public void sendVuzixMessage(String peerId, String message, final ActionCallback<Void> callback) {
        if (socketIOClient == null) {
            Log.e(TAG, "socketIOClient is not established.");
            return;
        }
        JSONObject jsonObject = new JSONObject();
        try {
            jsonObject.put("to", peerId);
            jsonObject.put("data", message);
            socketIOClient.emit(VUZIX_MESSAGE_TYPE, jsonObject);
            if (callback != null) {
                callback.onSuccess(null);
            }
        } catch (JSONException e) {
