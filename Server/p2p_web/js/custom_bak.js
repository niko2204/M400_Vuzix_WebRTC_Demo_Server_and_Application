...
jQuery(document).ready(function () {
   ...
   $('#devid').val("VuzixHud");
   $('#uid').val("portal");
   $('#remote-uid').val("VuzixHud");

   // needs to be updated when the remote ID changes
...
*/
   ...
   $('#target-disconnect').click(function () {
      p2p.stop($('#remote-uid').val()); // Stop chat
   });
   ...
   var options = {
      ...
      // content
      text: '{"ipaddr":"' + getUrlParameter('ipaddr') + '","port":8095,"devid":"' + $('#devid').val() + '","peerid":"' + $('#uid').val() + '"}',
      size: 100,
   ...
   $('#devname').click(function () {
      options.text = '{"ipaddr":"' + getUrlParameter('ipaddr') + '","port":8095,"devid":"' + $('#devid').val() + '","peerid":"' + $('#uid').val() + '"}';
      $('#mConnectQRCode').empty().qrcode(options);
   });

   $('#mConnectQRCode').qrcode(options);
});